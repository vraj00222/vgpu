...
- Every `HostShareableLayout` uses `layoutMode: "wgsl-host-shareable-v1"` and reports intrinsic WGSL alignment, size, member offsets, array stride, and matrix stride. A layout does not contain an address space: read `BindingInfo.addressSpace` for `uniform` versus `storage`. Address-space validation may reject a use, but never changes its reflected bytes.
