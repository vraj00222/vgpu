...
summary: Read before opening a PR or calling a render done. Gate correctness, measure, adopt the free performance defaults, and offer (never impose) cheaper alternatives with a number behind each one.
keywords: production, ship, shipping, pre-pr, before pr, pull request, checklist, done, finishing, release, review, optimize before merge, performance review, cheaper version, low tier, bundle size, first paint, pre-warm, compile, bundles, measure, vgpu check, browser test
relatedSymbols:
  - Frame
  - frameLoop
  - Effect
