...
description: "Start every visitor on the High pipeline, then downgrade once to a cheaper Low pipeline when detect-gpu, the Battery Status API, or presented-frame health says the device cannot hold it. Copy the code from the adaptive-quality example."
---
